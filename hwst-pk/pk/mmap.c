...
uintptr_t pk_vm_init()
{
  ...
  //printk("pk_vm_init()\n");
  //printk("mem_size: %ld, mem_pages: %ld, free_pages: %ld\n", mem_size, mem_pages, free_pages);
  //kenny increase page size from 8 to 2048 and again to 16384
  //free_pages = MAX(2048, mem_pages >> (RISCV_PGLEVEL_BITS-1));
  //free_pages = MAX(16384, mem_pages >> (RISCV_PGLEVEL_BITS-1));
  ...
  //original stack allocation for 1/3 of entire memory
  ...
  //new stack allocation for 1/2 of entire memory with spatial metadata compression
  //size_t stack_bottom = __do_mmap((current.mmap_max/2) - stack_size - ((current.mmap_max/2 - stack_size) & (RISCV_PGSIZE-1)), stack_size, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS|MAP_FIXED, 0, 0); // (addr & (RISCV_PGSIZE-1)) is calculation for  page aligned
  ...
  printk("kenny kernel shadow_space: %lx\n", shadow_space);
  
  //printk("current.stack_top: \t%ld\nstack_bottom: \t%ld\nshadow_space: \t%ld\n", current.stack_top, stack_bottom, shadow_space);
  //printk("stack_size: \t%ld\nshadow_size: \t%ld\n", stack_size, current.mmap_max - current.stack_top);
...
New:
      ...
      |      | STACK
      |======| <- current.mmap_max/3 - stack_size = stack_bottom
