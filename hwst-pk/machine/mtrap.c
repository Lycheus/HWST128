...
static void machine_page_fault(uintptr_t* regs, uintptr_t mcause, uintptr_t mepc)
{
  ...
  if (read_csr(mstatus) & MSTATUS_MPRV) {
    if (regs[11] == (MSTATUS_MPRV | MSTATUS_MXR)) {
      ...
      else if (mcause == CAUSE_OUT_OF_BOUNDS)
        write_csr(mcause, CAUSE_OUT_OF_BOUNDS);
      else if (mcause == CAUSE_KEY_MISMATCH)
	write_csr(mcause, CAUSE_KEY_MISMATCH);
      else
        goto fail;
...
//Tuo: the OOB trap bug is here. It's not delegated by PK but machine mode hypervisor
void trap_from_machine_mode(uintptr_t* regs, uintptr_t dummy, uintptr_t mepc)
{
  ...
  switch (mcause)
  {
    ...
    case CAUSE_OUT_OF_BOUNDS:
    case CAUSE_KEY_MISMATCH:
