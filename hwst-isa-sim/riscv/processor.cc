...
void processor_t::set_csr(int which, reg_t val)
{
  ...
  switch (which)
  {
	...
	(1 << CAUSE_OUT_OF_BOUNDS) |
	(1 << CAUSE_KEY_MISMATCH)
	;
...
void check_bounds(reg_t addr, reg_t upper, reg_t lower, reg_t pc)
{
  ...
  */
  ...
  if (addr < lower || addr >= upper){
    ...
    //reg_t pc = state.pc;
    printf("%08lX: kenny error SPATIAL memory violation %lx(addr)\t %lx(lower)\t %lx(upper)\n", pc, addr, lower, upper);
    throw trap_out_of_bounds(addr);
  }
}

void check_key(reg_t key, reg_t lock, reg_t key_in_lock, reg_t pc)
{
  if (key != key_in_lock){
    printf("%08lX: kenny error TEMPORAL memory violation %lx(lock)\t %lx(lock.key)\t %lx(key)\n", pc, lock, key_in_lock, key);
    throw trap_key_mismatch(lock);
  }
}
